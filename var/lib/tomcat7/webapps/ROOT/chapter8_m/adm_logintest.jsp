...
//자기 자신의 url을 써줘야 한다.
	...
	String loginOK=null;
	String jumpURL="adm_login.jsp?jump=adm_logintest.jsp";
		...
		<%for(int i=0;i<4;i++){
			...
			if(i==0){
				...
				if (found){
					...
					<a href = "d_oneview.jsp?resv_date=<%=resv_date%>&room=1"><%=rset.getString(1)%></a>
					<%}%>
				<%}else{%>
				...
				if (found){
					...
					<a href = "d_oneview.jsp?resv_date=<%=resv_date%>&room=2"><%=rset.getString(1)%></a>
					<%}%>
				<%}else{%>
				...
				if (found){
					...
					<a href = "d_oneview.jsp?resv_date=<%=resv_date%>&room=3"><%=rset.getString(1)%></a>
					<%}%>
				<%}else{%>
