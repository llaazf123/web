...
<%		
		...
		for(int k=1;k<100;k++){
			...
			if(fromPT>=((startpage*100)-99)&&fromPT<=(startpage*100)){
				...
				if(startpage==1){
					out.println("<a href = '?contentPage=e_01_list.jsp&fromPT=1&cntPT=10'><<</a>");
				}else {
					out.println("<a href = '?contentPage=e_01_list.jsp&fromPT=" + ((startpage*100)-199) +  "&cntPT=10'><<</a>");
				}
				//가운데 번호(10개씩 반복이니까 if문과 for문 조건은 변경 할 필요 없다. else도 마찬가지.)
				if(maxx<startpage*10+1){
					for(int i=((startpage*10)-9);i<maxx+1;i++){	
							out.println("<a href = '?contentPage=e_01_list.jsp&fromPT=" + ((i*10)-9) + "&cntPT=10'>" + i + "</a>");				
					}
				//11페이지, 21페이지, 31페이지일 때 else로 이동(11, 21, 31이 있어야 >>버튼이 생기기 때문)	
					...
					for(int i=((startpage*10)-9);i<(startpage*10)+1;i++){	
							out.println("<a href = '?contentPage=e_01_list.jsp&fromPT=" + ((i*10)-9) + "&cntPT=10'>" + i + "</a>");
					//>>버튼
					}out.println("<a href = '?contentPage=e_01_list.jsp&fromPT=" + ((startpage*100)+1) +  "&cntPT=10'>>></a>");
				}
			}        
		}
		%>	
