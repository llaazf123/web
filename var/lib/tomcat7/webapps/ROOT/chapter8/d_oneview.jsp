...
<script type="text/javascript">		
	//각 항목을 입력하지 않았을 때 해당 항목에 맞는 alert를 띄우는 함수
	function submitForm(mode) {
	   if(mode == 'write') {
		  ...
		  if(name.value.trim().length == 0){
		   ...
		   fm.action = "?contentPage=d_update.jsp";  
	   }
	   fm.submit();
   } 
   	// 입력값 제외 function
...
<%
	...
	//오늘날짜와 오늘날짜에 30일을 더한 값을 각 각 변수로 지정
...
<table border=1 align=center style="width:100%;" height="800" >
	<tr>
		...
		<!--한글, 영어만 입력가능하게 함 + 한글을 우선적으로 작성할 수 있게 함-->
		<td><input type="text" name="name" id="name" style="width:100%;height:100%" maxlength="10" value="<%=rset.getString(1)%>" style="ime-mode:active" 
		onkeypress='onlyHanEng()'></td>	
	</tr>
	<tr>
	...
	<tr>
		...
		<td><input type="text" name="phone" id="phone" style="width:100%;height:100%" value="<%=rset.getString(5)%>" maxlength="15" 
		onKeyup="this.value=this.value.replace(/[^0-9]/g,'');"></td>
	</tr>
	<tr>
		<td style="text-align:center;height:6%;font-weight: bold;" bgcolor="#f5f5f5">입금자명</td>
		<!--한글, 영어만 입력가능하게 함 + 한글을 우선적으로 작성할 수 있게 함-->
		<td><input type="text" name="depositor" id="depositor" style="width:100%;height:100%" value="<%=rset.getString(6)%>" style="ime-mode:active" 
		onkeypress='onlyHanEng()' maxlength="10"></td>
	</tr>
	<tr>
	...
	<tr style="border-right: hidden;border-left:hidden;border-bottom:hidden">
		<td colspan="2" align=center>
			<input class="btn btn-secondary" type = "button" value = "관리자 페이지" onclick="location.href='?contentPage=d_list.jsp'">
			<input class="btn btn-warning" type="button" value="수정" onclick='submitForm("write")'>
			<input class="btn btn-danger" type="submit" value="삭제" formaction="?contentPage=d_delete.jsp">
		</td>
	</tr>
</table><br><br><br><br>
<%
