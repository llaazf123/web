		...
		<%for(int i=0;i<4;i++){
			...
			if(i==0){
				...
				if (found){
					...
					<a href = "?contentPage=adm_oneview.jsp&resv_date=<%=resv_date%>&room=1"><%=rset.getString(1)%></a>
					<%}%>
				<%}else{%>
					<a href = "?contentPage=d_02.jsp&resv_date=<%=resv_date%>&room=1&adm=1">예약가능</a>
				<%}
			}else if(i==2){
				...
				if (found){
					...
					<a href = "?contentPage=adm_oneview.jsp&resv_date=<%=resv_date%>&room=2"><%=rset.getString(1)%></a>
					<%}%>
				<%}else{%>
					<a href = "?contentPage=d_02.jsp&resv_date=<%=resv_date%>&room=2&adm=1">예약가능</a>
				<%}
			}else{
				...
				if (found){
					...
					<a href = "?contentPage=adm_oneview.jsp&resv_date=<%=resv_date%>&room=3"><%=rset.getString(1)%></a>
					<%}%>
				<%}else{%>
					<a href = "?contentPage=d_02.jsp&resv_date=<%=resv_date%>&room=3&adm=1">예약가능</a>
				<%}
				cal.add(cal.DATE,+1);
			}%>
			</td>
